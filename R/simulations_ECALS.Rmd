...
# n_subjects_per_group <- 50
# eff_probs <- rep(c(0.5, 0.7, 0.8), times = 20)
# keep_fits <- TRUE

# Final scenario
questions_to_fit <- question_col_names
n_subjects_per_group <- 50
eff_probs <- rep(c(0.5, 0.7, 0.8), times = 100)
keep_fits <- TRUE
...
if(keep_fits) {
  ...
  all_effects_logit <- purrr::map_df(1:length(eff_probs), 
    ...
    summaries_logit <- brm_parallel(args_shared = list(formula = bf(proact_formula, family = cumulative()) + set_rescor(FALSE), adapt_delta = 0.95, init = 0.1, chains = 2, iter = 3000, warmup = 1000), args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE, summarise_fun = effect_from_sim_study,
                                    future.globals = list("effect_from_sim_study" = effect_from_sim_study))
    
    all_effects_logit <- purrr::map_df(1:length(eff_probs), 
                             ~ data.frame(id = .x, type = "logit", eff_prob = eff_probs[.x], estimate = summaries_logit[[.x]]))
}
```
