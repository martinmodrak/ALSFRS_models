...
```{r}
make_stancode(bf(proact_formula, family = cumulative()) + set_rescor(FALSE),
              data = args_per_fit[[1]]$data)


args_per_fit[[1]]$data$subject_id %>% unique() %>% length()
```

```{r}
## Sum to zero test
# data_s0 <- make_standata(bf(proact_formula, family = cumulative()) + set_rescor(FALSE),
#               data = args_per_fit[[1]]$data)
# 
# model_s0 <- cmdstan_model("../stan/indep_logit_sum0.stan")
# 
# fit_s0 <- model_s0$sample(data = data_s0, init = 0.1)
